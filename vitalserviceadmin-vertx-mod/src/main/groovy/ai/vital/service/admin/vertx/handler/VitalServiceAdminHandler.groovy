...
class VitalServiceAdminHandler extends AbstractVitalServiceHandler {
	...
	public VitalServiceAdminHandler(Vertx vertx) {
		super(vertx);
	}
	...
	protected Object handleMethod(String method, Object[] a) {
		...
		if(method == 'addApp') {
			...
			if(
				! checkParams(method, a, false, App.class, URIProperty.class) &&
				! checkParams(method, a, false, App.class, List.class)
			) {
				throw new RuntimeException("${method} expects App and either a ${URIProperty.class.simpleName} or a list of ${URIProperty.class.simpleName} objects")
			}
			...
			if( checkParams(method, a, false, App.class, URIProperty.class) ) {
				response = service.deleteExpanded(a[0], a[1])
			} else if(checkParams(method, a, false, App.class, List.class) ) {
				response = service.deleteExpanded(a[0], a[1])
			} else if(checkParams(method, a, false, App.class, URIProperty.class, VitalPathQuery.class)
			|| checkParams(method,a, false, App.class, URIProperty.class, String.class)
			) {
				queryFilter(a, 2)
				response = service.deleteExpanded(a[0], a[1], a[2])
			} else {
				throw new RuntimeException("${method} expects App and either a ${URIProperty.class.simpleName} or a list of ${URIProperty.class.simpleName} objects or an ${URIProperty.class.simpleName},${VitalPathQuery.class.simpleName} ")
			}

			
		} else if(method == 'deleteExpandedObject') {
		
			checkParams(method, a, false, App.class, GraphObject.class)
				
			response = service.deleteExpandedObject(a[0], a[1])
			
		} else if(method == 'deleteExpandedObjects') {

			queryFilter(a, 2)
					
			checkParams(method, a, false, App.class, List.class, VitalPathQuery.class)
			
			response = service.deleteExpandedObjects(a[0], a[1], a[2])
...
//				response = service.deleteFile(a[0], a[1], a[2])
			...
			checkParams(method, a, true, App.class, GraphObject.class)
			
			response = service.deleteObject(a[0], a[1])
			...
			checkParams(method, a, true, App.class, ServiceOperations.class)
...
//				response = service.fileExists(a[0], a[1], a[2])
			...
			if(
				checkParams(method, a, false, App.class, GraphContext.class, List.class) ||
				checkParams(method, a, false, App.class, GraphContext.class, URIProperty.class) ) {
				
				response = service.get(a[0], a[1], a[2])
				
			} else if(
				checkParams(method, a, false, App.class, GraphContext.class, List.class, Boolean.class) ||
				checkParams(method, a, false, App.class, GraphContext.class, List.class, List.class) ||
				checkParams(method, a, false, App.class, GraphContext.class, URIProperty.class, Boolean.class) ||
				checkParams(method, a, false, App.class, GraphContext.class, URIProperty.class, List.class)
			) {
			...
			if( checkParams(method, a, false, App.class, URIProperty.class, Boolean.class) ) {
				
				response = service.getExpanded(a[0], a[1], a[2])
				
			} else if( checkParams(method, a, false, App.class, URIProperty.class, VitalPathQuery.class, Boolean.class) ) {
				...
				throw new RuntimeException("${method} expects App, URIProperty, Boolean or URIProperty, VitalPathQuery, Boolean")
			...
			if( checkParams(method, a, false, App.class, VitalSegment.class, GraphObject.class) 
				|| checkParams(method, a, false, App.class, VitalSegment.class, List.class) ) {
				
				response = service.insert(a[0], a[1], a[2])
			} else {
...
//				response = service.listFiles(a[0], a[1])
			...
			checkParams(method, a, true, App.class, VitalQuery.class)
			...
			checkParams(method, a, true, App.class, VitalQuery.class, List.class)
			...
			if(
				...
				checkParams(method, a, false, App.class, VitalSegment.class, GraphObject.class, Boolean.class) ||
				checkParams(method, a, false, App.class, VitalSegment.class, List.class, Boolean.class)
			){
				response = service.save(a[0], a[1], a[2], a[3])
			} else {
