...
class VitalAuthManager extends GroovyVerticle {

	private final static Logger log = LoggerFactory.getLogger(VitalAuthManager.class)
...
//	
	...
	static class AuthAppBean {
		...
		protected void _initService(Vertx vertx) {
			...
			vitalServiceSync = VitalServiceVertx3.registeredServices.get(appID)
		...
		protected void _passMessage(Message msg) {
			...
			vertx.eventBus().send(vitalService.address, msg.body()) { Future<Message> response ->
...
//			vitalService.address
	...
	void start(Future<Void> startedResult) {
		...
		synchronized(VitalAuthManager.class) {
			...
			if(initialized) {
				startedResult.complete(new Object())
				return
			}
		...
		def cfg = context.config();
		Object appsMapO = context.config().get('apps')
		...
		for(Entry<String, Object> e : appsMap.entrySet()) {
			...
			log.info("App: ${appID} access: ${access}")
			...
			if(authEnabled) {
				...
				if(access == 'service') {
					...
					if(!bean._supportsAdmin()) throw new RuntimeException("This verticle does not support admin mode");
					...
					if(!bean._supportsSuperAdmin()) throw new RuntimeException("This verticle does not support super admin mode");
				...
				log.info("App: ${appID} max sessions per user: ${bean.maxSessionsPerUser}")
				...
				log.info "App: ${appID} session Timeout: ${bean.sessionTimeout}"
				...
				log.info("App: ${appID} expirationProlongMargin: ${bean.expirationProlongMargin}")
		...
		eb.consumer(getAddress_login()) { Message message ->
			doLogin(message);
		}
		
		eb.consumer(getAddress_logout()) { Message message ->
			doLogout(message);
		}
		
		eb.consumer(getAddress_authorise()) { Message message ->
			doAuthorise(message);
		}
	...
	protected void logout(AuthAppBean bean, Message message, String sessionID, Closure callback) {
		...
		if(bean.persistentSessions) {
			...
			if(type == Login.class.simpleName) {
				...
				log.error("invalid session type: ${type}")
				callback(false)
				return
			}
